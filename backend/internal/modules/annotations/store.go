...
func (s Store) Create(ctx context.Context, userID, paperID uuid.UUID, page int, rect *Rect, text, note, color string) (Out, error) {
	var raw []byte
	if rect != nil {
		raw, _ = json.Marshal(rect)
	}
	return s.scan(ctx, `INSERT INTO annotations(id,paper_id,user_id,page,rect,selected_text,note,color) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id,paper_id,page,rect,selected_text,note,color,created_at,updated_at`, uuid.New(), paperID, userID, page, raw, text, note, color)
}

func (s Store) Patch(ctx context.Context, userID, id uuid.UUID, note string) (Out, error) {
	return s.scan(ctx, `UPDATE annotations SET note=$3,updated_at=now() WHERE id=$1 AND user_id=$2 RETURNING id,paper_id,page,rect,selected_text,note,color,created_at,updated_at`, id, userID, note)
}
