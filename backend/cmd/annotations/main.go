...
func main() {
	...
	r.Get("/health", func(w http.ResponseWriter, _ *http.Request) {
		httpx.JSON(w, 200, map[string]string{"status": "ok", "service": "annotations"})
	})
	r.Group(func(r chi.Router) {
		r.Use(identity.MiddlewareFromGateway)
		annotations.API{Store: annotations.Store{DB: pool}, Papers: papers}.Mount(r)
	})

	log.Printf("annotations listening on %s", cfg.HTTPAddr)
