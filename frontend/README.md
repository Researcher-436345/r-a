...
В веб- и глубоком поиске новые фрагменты прокручивают только сам чат и только
пока пользователь находится внизу. Прокрутка вверх отключает следование;
возвращение в самый низ включает его снова. Для ручной проверки постепенного
роста ответа добавьте `FIXTURE_STREAM_CHUNK_CHARS=50` к настройкам fixture API.
