...
## Фоновые ответы при переходах

Веб-поиск, глубокий поиск и чат статьи хранят сообщения и состояние генерации
по идентификатору диалога вне React-страницы. Переход в другой чат, статью или
библиотеку не отменяет запрос. При возвращении виден текущий ответ; поздняя
загрузка истории не может затереть более новые фрагменты. Выход из аккаунта
очищает локальное хранилище. Это относится к навигации внутри открытого
приложения, а не к перезагрузке или закрытию вкладки.

Проверка: `node --test scripts/test-conversation-store.mjs`.
Для ручной проверки запустите fixture API с
`FIXTURE_STREAM_DELAY_SECONDS=25 python3 scripts/v3-fixture-server.py`,
отправьте вопрос и перейдите в другой раздел до завершения ответа.
